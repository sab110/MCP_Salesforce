...
#         base_url = os.getenv("SF_INSTANCE_URL", "").rstrip("/")

#         if not base_url:
#             raise ValueError("SF_INSTANCE_URL not set in environment variables.")

#         if not records:
#             return [types.TextContent(type="text", text="There were no records found for your query.")]
...
#             name = rec.get("Name", record_id)

#             if obj_type and record_id:
#                 url = f"{base_url}/lightning/r/{obj_type}/{record_id}/view"
#                 display = f"🔗 [Open {obj_type} - {name}]({url})"
#                 outputs.append(display)
#             else:
#                 outputs.append(f"• {json.dumps(rec)}")
...
def run_soql_query_impl(sf_client: OrgHandler, arguments: dict[str, str]):
    ...
    try:
        ...
        # First output: full JSON result for internal use or summarization
        outputs = [
            types.TextContent(
                type="text",
                text=f"SOQL Query Results (JSON):\n{json.dumps(results, indent=2)}"
        ...
        # Second output: clickable URLs for each record (if applicable)
        if records:
            links = []
            for rec in records:
                obj_type = rec.get("attributes", {}).get("type")
                ...
                if obj_type and record_id:
                    url = f"{base_url}/lightning/r/{obj_type}/{record_id}/view"
                    links.append(f"🔗 [Open {obj_type} - {name}]({url})")
                else:
                    links.append(f"• {json.dumps(rec)}")
            ...
            outputs.append(
                types.TextContent(
                    type="text",
                    text="There were no records found for your query."
                )
            )
...
def get_object_fields_impl(sf_client: OrgHandler, arguments: dict[str, str]):
    ...
    try:
        ...
        results = sf_client.connection.__getattr__(object_name).describe()["fields"]
        return [
            types.TextContent(
                type="text",
